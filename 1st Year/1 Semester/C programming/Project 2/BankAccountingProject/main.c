...
void salvare_client(char nume[100], char prenume[100], char cnp[100], int varsta) {
    ...
    nrclienti++;
    strcat(cnpuri,clienti[nrclienti].CNP);
    strcat(cnpuri," ");
}
...
//exportare intr un fisier CSV
void exportToCSV() {
    FILE *csvFile = fopen("financial_data.csv", "w");
    if (csvFile == NULL) {
        printf("Error opening CSV file for writing.\n");
        return;
    }

    fprintf(csvFile, "----Transactioe,Descriere,Suma-----\n");

    for (int i = 0; i < nrtranzactii; i++) {
        fprintf(csvFile, "%s,%s,%.2f\n", tranzactii[i].descriere, tranzactii[i].tip2, tranzactii[i].suma);
    }
...
void salveazaAudit() {
    FILE *auditFile = fopen("Audit.txt", "w");
    if (auditFile == NULL) {
        printf("Eroare la deschiderea fisierului Audit.txt pentru scriere.\n");
        return;
    }

    // Salvează conturile
    fprintf(auditFile, "=== Conturi ===\n");
    for (int i = 0; i < nrconturi; i++) {
        fprintf(auditFile, "%d %s %s\n", conturi[i].index, conturi[i].nume, conturi[i].parola);
    }

    // Salvează clienții
    fprintf(auditFile, "\n=== Clienti ===\n");
    for (int i = 0; i < nrclienti; i++) {
        fprintf(auditFile, "%d %s %s %s %d\n", clienti[i].index, clienti[i].nume, clienti[i].prenume, clienti[i].CNP, clienti[i].varsta);
    }

    // Salvează tranzacțiile
    fprintf(auditFile, "\n=== Tranzactii ===\n");
    for (int i = 0; i < nrtranzactii; i++) {
        fprintf(auditFile, "%d %d %s %s %.2f\n", tranzactii[i].index, tranzactii[i].index2, tranzactii[i].descriere, tranzactii[i].tip2, tranzactii[i].suma);
    }
